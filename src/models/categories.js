...
const countCategory = (data, cb) => {
    db.query(`SELECT COUNT(*) AS total FROM ${categories} WHERE type LIKE '${data.search}%';`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
...
const getCategories = (data, cb) => {
    db.query(`SELECT * FROM ${categories} WHERE type LIKE '${data.search}%' LIMIT ${data.limit} OFFSET ${data.offset}`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
...
const countCategoryData = (data, cb) => {
    db.query(`SELECT COUNT(*) AS total FROM ${categories} c LEFT JOIN vehicles v ON c.id_category=v.id_category WHERE c.type LIKE '${data.search}%';`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
...
const getCategoriesData = (data, cb) => {
    db.query(`SELECT c.id_category, c.type, v.brand, v.capacity, v.location, v.price, v.qty, v.rent_count, c.createdAt, c.updatedAt FROM ${categories} c LEFT JOIN vehicles v ON c.id_category=v.id_category WHERE c.type LIKE '${data.search}%' LIMIT ${data.limit} OFFSET ${data.offset}`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
...
const getTypeIdCategories = (cb) => {
    db.query(`SELECT id_category, type FROM ${categories}`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
