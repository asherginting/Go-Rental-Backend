...
exports.requestPassword = (data)=>new Promise((resolve, reject)=>{
    db.query(`INSERT INTO forgot_request (user_id, code, expired_date) VALUES(${data.user_id}, ${data.code}, ${data.expired_date})`, (err, res)=>{
        if(err) reject(err);
        resolve(res);
    });
});
...
exports.updateReqPassword = (id)=>new Promise((resolve, reject)=>{
    db.query('UPDATE forgot_request SET status = 0 WHERE id=?', [id], (err, res)=>{
        if(err) reject(err);
        resolve(res);
    });
});
