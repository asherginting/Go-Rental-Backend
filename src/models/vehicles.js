...
const countVehicleCategory = (data, cb) => {
    ...
    db.query(`SELECT COUNT(*) as total FROM ${vehicles} v LEFT JOIN ${categories} c ON c.id_category=v.id_category WHERE v.location LIKE '${data.location}%' AND (v.brand LIKE '${data.search}%' OR c.type LIKE '${data.search}%') AND (v.price >= ${data.minimum} AND v.price <= ${data.maximum}) AND payment >= ${data.prepayment} ${sort}`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
...
const checkVehicle = (data, cb) => {
    db.query(`SELECT * FROM ${vehicles} WHERE type='${data.type}' AND brand='${data.brand}' AND capacity='${data.capacity}' AND location='${data.location}' AND price=${data.price} AND qty=${data.qty}`, (err, res) => {
        if (err) throw err;
        cb(res);
    });
};
